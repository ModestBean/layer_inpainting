...
def saveLabelStackToImagesX(labelStack, outputDir, ext=".png"):
    depth = labelStack.shape[0]
    mkoutdir(outputDir)
...
def saveLabelStackToImagesY(labelStack, outputDir, ext=".png"):
    depth = labelStack.shape[1]
    mkoutdir(outputDir)
    for i in range(depth):
        fileName = str(i).zfill(3) + ext
        image = labelStack[:, i, :]
...
def saveLabelStackToImagesZ(labelStack, outputDir, ext=".png"):
    """
...
def savaLabelStackToMat(labelStack, outputDir):
    """
    功能：将三维label矩阵存储为二值mat
    :param labelStack:三维label矩阵
    :param outputDir:输出路径（包括文件名），例：.\result\001.mat
    :return:
    """
    utils.mat.save_mat(outputDir, labelStack)
...
if __name__ == "__main__":
    ...
    # labelStack = getLabelStackFromMat(inputAddress)
    # outputDirX = ".\\result\\slicerX\\"
    # outputDirY = ".\\result\\slicerY\\"
    # outputDirZ = ".\\result\\slicerZ\\"
    # saveLabelStackToImagesX(labelStack, outputDirX)
    # saveLabelStackToImagesY(labelStack, outputDirY)
    # saveLabelStackToImagesZ(labelStack, outputDirZ)
